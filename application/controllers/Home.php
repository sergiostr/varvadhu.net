    ...
    function profile($para1="",$para2="",$para3="")
    {
        ...
        elseif ($para1=="update_all") {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $data['height'] = $this->input->post('user_height');
                ...
                $data['mobile2'] = $this->input->post('mobile2');
                ...
                $basic_info[] = array(
                                    ...
                                    'caste'                 =>  $this->input->post('caste'),
                                    'sub_caste'             =>  $this->input->post('sub_caste'),
                                    'p_height'              =>  $this->input->post('user_height'),
                ...
                // ------------------------------------Education & Career------------------------------------ //
                $education_and_career[] = array('education'         =>  $this->input->post('education'),
                ...
                // ------------------------------------Family Information------------------------------------ //
                $family_info[] = array('family_tyle'        =>  $this->input->post('family_type'),
                                    'family_values'         =>  $this->input->post('family_status'),
        ...
        elseif ($para1=="update_basic_info") {
            ...
            $this->form_validation->set_rules('on_behalf', 'On Behalf', 'required');
            $this->form_validation->set_rules('date_of_birth', 'Date of Birth', 'required');
            $this->form_validation->set_rules('child_count', 'Number of Children', 'required');
            $this->form_validation->set_rules('mobile', 'Mobile', 'required');
            $this->form_validation->set_rules('caste', 'Caste', 'required');
            $this->form_validation->set_rules('sub_caste', 'Gotra', 'required');
            $this->form_validation->set_rules('user_height', 'Height', 'required');
            $this->form_validation->set_rules('religion', 'Religion', 'required');
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $data['caste'] = $this->input->post('caste');
                $data['marital_status'] = $this->input->post('marital_status');
                $data['subcaste'] = $this->input->post('sub_caste');
                $data['height'] = $this->input->post('user_height');
                $data['religion'] = $this->input->post('religion');
                $data['date_of_birth'] = strtotime($this->input->post('date_of_birth'));
        ...
        elseif ($para1=="update_education_and_career") {
           ...
           // else {
                ...
                $data['eduction_detail'] = $this->input->post('education_detail');
                $data['occupation'] = $this->input->post('occupation');
                $data['occupation_detail'] = $this->input->post('occupation_detail');
        ...
        elseif ($para1=="update_family_info") {
            // ------------------------------------Family Information------------------------------------ //
            $family_info[] = array('family_type'            =>  $this->input->post('family_type'),
                                'family_values'             =>  $this->input->post('family_value'),
                                'family_status'             =>  $this->input->post('family_status'),
                                'mother_tounge'             =>  $this->input->post('mother_tounge'),
                                'father_name'               =>  $this->input->post('father_name'),
                                'father_occupation'         =>  $this->input->post('father_occupation'),
            ...
            $data['married_sisters'] = $this->input->post('married_sisters');
            $data['father_name'] = $this->input->post('father_name');
            $data['father_occupation'] = $this->input->post('father_ocuupation');
  ...
  function instamojo_success(){
        ...
        if ($this->member_permission() == TRUE) {
            ...
            elseif ($para1=="add_info") {
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    if($state == '1'){
                        $data['message'] = "Already same email eixts, Please try other email address";
                        echo "<script> alert('Already same email eixts, Please try other email address'); </script>";
    ...
    function ajax_search_list($para1="", $para2="")
    {
        ...
        elseif ($search_type == 'advance-search')
        {
            ...
            $where .= " AND TIMESTAMPDIFF(YEAR, from_unixtime(date_of_birth), NOW())>=".$from_age." AND TIMESTAMPDIFF(YEAR, from_unixtime(date_of_birth), NOW())<=".$to_age;
            if ($with_photo == 'on')
                $where .= " AND profile_image not like '%default_image.png%'";
            if ($id != '')
